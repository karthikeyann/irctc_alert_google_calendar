...
var CLIENT_ID = '182266680014-ui6o0uthgq9rovjulskdrqn96ai26q9k.apps.googleusercontent.com';
if (window.location.host == "localhost:8000") {
  CLIENT_ID = '182266680014-lkfkkr8g2j4mr8r3aq63v46j73vej1di.apps.googleusercontent.com';
} else if (window.location.host == "karthikeyann.github.io") {
...
function add1event(todayDate, id, reminderdays) {
  ...
  request.execute(function (event) {
    ...
    if (typeof event.htmlLink === 'undefined') {
      appendPre("Failed for " + todayDate.format() + event.error.message);
    } else {
      //console.log(todayDate.format() + " reminder link: " + event.htmlLink);
      appendPre('Event created: on ' + remindDate.format());
      appendLink(todayDate.format() + ":+120=" + remindDate.format(), event.htmlLink);
    }
  });
}
...
function appendLink(name, link) {
  var sidebar = document.getElementById("sidebar");
  sidebar.insertAdjacentHTML("beforeend", `<a href="${link}"><div>${name}</div></a>`)
}